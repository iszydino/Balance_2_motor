  ...
  <body>\
      ...
      <form id='pidForm' action='/setSpeed' method='GET'>\
          ...
          <p id='angleInfo' class='info'>Angles: " +
                String(roll) + "</p>\
          <p id='PInfo' class='info'>P: " +
                String(pid_motor.P) + "</p>\
          <p id='IInfo' class='info'>I: " +
                String(pid_motor.I) + "</p>\
          <p id='DInfo' class='info'>D: " +
                String(pid_motor.D) + "</p>\
          <!-- Nut gui -->\
      ...
      <script>\
          ...
          async function getData(){\
            ...
            let P = parseFloat(parts[5]);\
            let I = parseFloat(parts[7]);\
            let D = parseFloat(parts[9]);\
            cube.rotation.z= (roll * 3.142 / 180);\
            document.getElementById('outInfo').innerText = 'Out: ' + output;\
            document.getElementById('angleInfo').innerText = 'Angles: ' + roll.toFixed(2);\
            document.getElementById('PInfo').innerText = 'P: ' + P;\
            document.getElementById('IInfo').innerText = 'I: ' + I;\
            document.getElementById('DInfo').innerText = 'D: ' + D;\
          }\
...
void handleData()
{
  String message = "Roll:" + String(roll) + ":output:" + String(pid_motor.output) + ":P:" 
  + String(pid_motor.P) + ":I:" + String(pid_motor.I) + ":D:" + String(pid_motor.D); 
